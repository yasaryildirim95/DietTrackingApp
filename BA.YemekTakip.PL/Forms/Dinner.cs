...
namespace BA.DietTrackApp.PL.Forms
{
    public partial class Dinner : Form
    {
        ...
        {
            // Öğle yemeği düğmesi devre dışı bırakılıyor.
            mainForm.buttonDinner.Enabled = false;
            ...
            data2 = foodMealManager.GetTablewMT().Where(x => x.Meal.UserId == mainForm.tempint && x.Meal.Date == mainForm.monthCalendar1.SelectionStart.Date && x.Meal.MealType.Id == 2).ToList();
            ...
            var data3 = foodMealManager.GetTablewMT().Where(x => x.Meal.UserId == mainForm.tempint && x.Meal.Date == mainForm.monthCalendar1.SelectionStart.Date && x.Meal.MealType.Id == 2).Sum(x => x.Food.Calori);
        ...
        private void Dinner_FormClosed(object sender, FormClosedEventArgs e)
        {
            // Öğle yemeği penceresi kapatıldığında ana formdaki öğle yemeği düğmesi etkinleştiriliyor.
